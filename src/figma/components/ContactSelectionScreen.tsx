      ...
      <div className="relative z-10 flex-1 flex flex-col min-h-0">
            ...
            {contacts.map((contact) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  {/* Contact name */}
                  <span className="text-black font-medium chat-text text-lg">{contact.name}</span>
                </div>

                {/* Selection indicator */}
                <div
                  className={`w-6 h-6 rounded-full border flex items-center justify-center ${
                    contact.selected ? 'bg-[#642975] border-[#642975]' : 'border-[#642975]'
                  }`}
                >
                  {contact.selected && <Check size={14} className="text-white" />}
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="px-6 py-6 shrink-0">
          ...
          >
            {selectedCount === 0
              ? 'Add Contacts'
              : `Add ${selectedCount} ${selectedCount === 1 ? 'Ally' : 'Allies'}`}
          </button>
        </div>
      </div>
    </div>
