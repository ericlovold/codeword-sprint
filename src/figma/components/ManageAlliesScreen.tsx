    ...
    <div className="h-screen w-full flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 px-6 pb-4 shrink-0">
        <p className="text-gray-600 chat-text text-sm">
          ...
          {currentAllies.length > 0 &&
            ` Currently ${currentAllies.length} ally${currentAllies.length === 1 ? '' : 'ies'} selected.`}
        </p>
      </div>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto">
        <div className="px-6 pb-6">
          <h3 className="text-lg font-medium text-black chat-text mb-4">Select Your Allies</h3>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    <span className="font-medium text-black chat-text">{contact.name}</span>
                    {isCurrentAlly && (
                      <div
                        className={`text-xs chat-text ${isPendingInvite ? 'text-red-500' : 'text-gray-500'}`}
                      >
                  ...
                  <div
                    className={`w-6 h-6 rounded-full border flex items-center justify-center transition-all ${
                      isSelected ? 'border-[#642975] bg-[#642975]' : 'border-gray-300'
                    }`}
                  >
                    {isSelected && (
      ...
      <div className="relative z-10 px-6 py-6 shrink-0">
        ...
        >
          {selectedCount === 0
            ? 'Select at least one ally'
            : `Save ${selectedCount} ${selectedCount === 1 ? 'Ally' : 'Allies'}`}
        </button>
      </div>
    </div>
  );
